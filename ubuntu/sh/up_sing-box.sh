...
interactive_install() {
    ...
    case "$choice" in
        ...
        2)
            ...
            if [[ "$user_version" == "$CURRENT" ]]; then
                echo -e "${YELLOW}当前已安装的就是这个版本 (v${user_version})，是否重新安装？[y/N]: ${NC}"
                ...
                local cmp_result=$(compare_versions "$user_version" "$CURRENT")
                if [[ "$cmp_result" == "-1" ]]; then
                    echo -e "${YELLOW}⚠️ 警告：您要安装的版本 (v${user_version}) 低于当前版本 (v${CURRENT})${NC}"
                    echo -e "${YELLOW}这是降级操作，可能会导致功能缺失或不稳定。${NC}"
                    echo -e -n "${YELLOW}确认要降级吗？[y/N]: ${NC}"
                    read -r confirm </dev/tty
                    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
                        echo -e "${BLUE}已取消安装。${NC}"
                        exit 0
                    ...
                    echo -e "${GREEN}您要安装的版本 (v${user_version}) 高于当前版本 (v${CURRENT})${NC}"
                    echo -e -n "${YELLOW}确认要升级吗？[y/N]: ${NC}"
                    read -r confirm </dev/tty
                    if [[ ! "$confirm" =~ ^[Yy]$ ]]; then
                        echo -e "${BLUE}已取消安装。${NC}"
                        exit 0
